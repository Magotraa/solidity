...
bool SMTEncoder::analyze(SourceUnit const& _source)
{
	...
	set<FunctionCall const*> abiCalls = collectABICalls(&_source);
	for (auto const& source: _source.referencedSourceUnits(true))
		abiCalls += collectABICalls(source);
	m_context.state().buildABIFunctions(abiCalls);
...
set<FunctionCall const*> SMTEncoder::collectABICalls(ASTNode const* _node)
{
	struct ABIFunctions: public ASTConstVisitor
	{
		...
		{
			static set<FunctionType::Kind> const abiKinds{
			...
			{
				auto const& funType = dynamic_cast<FunctionType const&>(*_funCall.expression().annotation().type);
				if (abiKinds.count(funType.kind()))
